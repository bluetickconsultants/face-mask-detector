...
def preprocessing_and_training(EPOCHS,INIT_LR,BS):
	...
	path="dataset/"
	df=pd.DataFrame.from_csv("mnm_dataset.csv")
	...
	train_x = train_x.reshape(len(df), 222, 222,1).astype('float32')
	...
	model = CNNmodel.build(width=222, height=222, depth=1, classes=2)
